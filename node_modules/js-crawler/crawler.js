...
Crawler.prototype.crawl = function (url, onSuccess, onFailure) {
  this.crawlUrl(null,url, this.depth, onSuccess, onFailure);
};

Crawler.prototype.crawlUrl = function(parenturl,url, depth, onSuccess, onFailure) {
  ...
  request(url, function(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      self.crawlUrls(url,self.getAllUrls(url, body), depth - 1, onSuccess, onFailure);
    } else if (onFailure) {
	
		//Storing dead links here
		/*
		if(self.errorURLS[parenturl]){
			self.errorURLS[parenturl].push(url);
		}else{
			var a = [];
			a.push(url);
			self.errorURLS[parenturl]=a;
		}*/
		
		var s = "PARENT::" + parenturl + '\r\n' + "DEADLINK::" + url + '\r\n' + '\r\n';
		
		fs.appendFile('helloworld.txt', s, function (err) {
...
Crawler.prototype.crawlUrls = function(baseurl,urls, depth, onSuccess, onFailure) {
  var self = this;

  _.each(urls, function(url) {
    self.crawlUrl(baseurl,url, depth, onSuccess, onFailure);
  });
};
